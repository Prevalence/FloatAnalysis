...
def main(args):
    ...
    op.add_option('','--write-floats', dest='writeFloats', action='store_true',
                  default=False,
                  help='convert 4/8-byte sequences to floats/doubles')
    ...
    for file in args:
        ...
        for i,(name,data) in enumerate(b.objects):
            ...
            if opts.writeInts and len(data) == 4: 
                ...
                #print >> f, "%r" % (struct.unpack('i',str)[0])
                f.writelines(struct.unpack('i',str)[0]+'} ')
            elif opts.writeFloats and len(data) == 4: 
                print 'object %4d: data: %r' % (i, struct.unpack('f',str)[0])
                #print >> f, "%r" % (struct.unpack('f',str)[0])
                f.writelines(__builtins__.str(struct.unpack('f',str)[0])+'} ')
            elif opts.writeFloats and len(data) == 8: 
                print 'object %4d: data: %r' % (i, struct.unpack('d',str)[0])
                #print >> f, "%r" % (struct.unpack('d',str)[0])
                f.writelines(__builtins__.str(struct.unpack('d',str)[0])+'} ')
            else:
